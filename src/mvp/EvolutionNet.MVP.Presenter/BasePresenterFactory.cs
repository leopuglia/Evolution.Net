...
namespace EvolutionNet.MVP.Presenter
{
	public abstract class BasePresenterFactory : BaseFactory, IPresenterFactory
	{
		...
		public IPresenter<TO, T, IdT> GetPresenter<TO, T, IdT>(IView<TO, T, IdT> view) 
			where TO : ITo<T, IdT> 
			where T : IModel<IdT>
		{
		...
		public IListPresenter<TO, T, IdT> GetListPresenter<TO, T, IdT>(IListView<TO, T, IdT> view) 
			where TO : IListTo<T, IdT> 
			where T : IModel<IdT>
		{
			return IoCHelper.InstantiateObj<IListPresenter<TO, T, IdT>>(TYPE_NAME_SOURCE, view.GetType(),
																		TYPE_NAME_DEST, GetType(), view);
		}
