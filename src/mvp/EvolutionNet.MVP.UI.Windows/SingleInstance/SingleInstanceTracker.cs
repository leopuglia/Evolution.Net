...
namespace EvolutionNet.MVP.UI.Windows.SingleInstance
{
	...
	public class SingleInstanceTracker : IDisposable
	{
		...
		/// <param name="name">The unique name used to identify the application.</param>
		/// <param name="enforcerRetriever">The method which would be used to retrieve an ISingleInstanceEnforcer object when instantiating the new object.</param>
		/// <param name="args">Arguments for the form creator.</param>
		/// <exception cref="System.ArgumentNullException">name is null or empty.</exception>
		/// <exception cref="EvolutionNet.MVP.UI.Windows.SingleInstance.SingleInstancingException">A general error occured while trying to instantiate the SingleInstanceInteractor. See InnerException for more details.</exception>
		public SingleInstanceTracker(string name, SingleInstanceEnforcerRetriever enforcerRetriever, params object[] args)
		{
			...
			try
			{
				...
				if (enforcerRetriever != null)
				{
					...
					if (isFirstInstance)
					{
						...
						ipcChannel = new IpcServerChannel(name, name, serverProvider);
						...
						ISingleInstanceEnforcer enforcer = enforcerRetriever(args);
					...
					else
					{
						...
						ipcChannel = new IpcClientChannel(name, clientProvider);
						// Register the channel and get it ready for use
						ChannelServices.RegisterChannel(ipcChannel, false);
		...
		/// <exception cref="EvolutionNet.MVP.UI.Windows.SingleInstance.SingleInstancingException">The SingleInstanceInteractor has failed to send the message to the first application instance. The first instance might have terminated.</exception>
